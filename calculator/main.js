...
const costsFlat = document.getElementById('costs-flat'),
    ...
    costsTransport1 = document.getElementById('costs-transport'),
...
for (input of inputs) {
    input.addEventListener('input', () => {
        coutingAvaliableMoney();
        calculationPrecents();
    })
}

//перегоняет incomeSalary.value в число (изначально строка) в coutingAvaliableMoney()

const strToNum = str => str.value ? parseInt(str.value) : 0

const coutingAvaliableMoney = () => {
    const totalPerMonth = strToNum(incomeSalary) + strToNum(incomeFreelance) + strToNum(incomeExtra1) + strToNum(incomeExtra2);
    const totalCosts = strToNum(costsFlat) + strToNum(costsHouseServices) + strToNum(costsTransport1) + strToNum(costsCredit);
